---
title: "User guide for the onlineChange R package"
author: "Ning Dai, Yi Rong, Galin Jones, Jie Ding"
date: "May 7, 2019"
output: 
...
vignette: >
  %\VignetteIndexEntry{User guide for the onlineChange R package}
...
Suppose the pre- and post-change models are RBMs with same numbers of visible layers and hidden layers. The differences between them are parameter values. We have a training set of the pre-change model $X_0$ and a training set of the pre-change model $X_1$, which we could use to train the RBMs and get the parameter values.
We use the package [RBM](https://github.com/TimoMatzen/RBM) to do this. We use the [MNIST](https://www.kaggle.com/c/mnist-digits/data), a hand-written numbers dataset as an example.
```{r,eval = FALSE}
# Load devtools
library(devtools)
# Install RBM
...
modelRBM.X0 <- RBM(x = trainX[train.Y0.index,][1:100,], n.iter = 1000, n.hidden = 50, size.minibatch = 10)
modelRBM.X1 <- RBM(x = trainX[train.Y1.index,][1:100,], n.iter = 1000, n.hidden = 50, size.minibatch = 10)
...
![](delay.pdf)![](computation.pdf)![](LowESS.pdf)![](acceptance.pdf)
...
# post- and pre-change log unnormalized probability functions, gradients and laplacians for normal distribution
ULP1=function(x,th) -(x-th[1])^2/2/th[2]^2
GLP1=function(x,th) -(x-th[1])/th[2]^2
LLP1=function(x,th) -1/th[2]^2
ULP0=function(x,th) -(x-th[1])^2/2/th[2]^2
